...
import { CjsCommonEvent, CjsCustomEvents, CjsEventTypes } from "./types";
...
type Mapping = {
    ...
    action: (event: Event, element: HTMLElement, data: any) => void;
...
class FunctionMappings {
    ...
    disabled: Map<string, DisabledMapping>;
    appliedFunctions: Map<string, AppliedFunction>;

    constructor() {
        this.mappings = new Map();
        this.disabled = new Map();
        this.appliedFunctions = new Map();
    }
    ...
    add(
        type: CjsCommonEvent,
        mappingFunction: (event: Event, element: HTMLElement, data: any) => void,
        options: MappingOptions = { windowApplied: false, additionalName: null },
        data: Record<string, any> = {}
    ): string {
        let attribute: string | null = null;

        while (attribute === null || this.mappings.has(attribute)) {
     ...
     */
    disable(event: CjsEventTypes[]): string {
        let attribute: string | null = null;

        while (attribute === null || this.disabled.has(attribute)) {
    ...
    getElementActionAttributes(
        element: HTMLElement,
        filterEventType: string | null = null,
        includeChildren = false
    ): string[] {
        const attributes: string[] = [];
        const elements = [
            element,
            ...(includeChildren ? (Array.from(element.children) as HTMLElement[]) : [])
        ];

        for (const child of elements) {
            const attributesStarting = getAttributeStartingWith(child, CJS_ELEMENT_PREFIX);
            ...
            if (filterEventType !== null) {
                ...
                attributesStarting.forEach(attr => attributes.push(attr));
            }
        }
    ...
    setData(attribute: string, data: Record<string, any>): void {
        if (!this.mappings.has(attribute)) {
            console.log(`${CJS_PRETTY_PREFIX_X}Cannot set data for ${Colors.Yellow}"${attribute}"${Colors.None}, because it doesn't exists`);
            return;
        }
    ...
    applyElementAttributeMappingFunction(
        element: HTMLElement,
        attribute: string,
        ...
        const targetElementEvent: HTMLElement | Window =
            mapping.options.windowApplied ? window : element;

        const eventFunction: EventListener = (event) => {
            ...
            if (parent !== null) {
                const startingAttributes = getAttributeStartingWith(
                    parent,
                    CJS_ELEMENT_DISABLED_PREFIX
                );
                ...
                for (const startingAttribute of startingAttributes) {
                    ...
                    const hasDisabledCommonEvent = data.events.includes(mapping.type as unknown as CjsEventTypes);
                    const hasDisabledAdditionalEvent =
                        mapping.options.additionalName !== null &&
                        data.events.includes(mapping.options.additionalName as any);
        ...
        if (this.appliedFunctions.has(attribute)) {
            ...
            lastApplied.element.removeEventListener(lastApplied.type, lastApplied.mappingFunction);
        }
    ...
    applyBodyMappings(): void {
        for (const element of document.body.querySelectorAll<HTMLElement>("*")) {
            this.applyElementMappingFunction(element, false);
        }
    }
}
