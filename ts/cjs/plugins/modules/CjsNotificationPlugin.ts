...
import { CjsTimings } from "../../utils/user-helpers/CjsTimingsUtil";
import { CjsPlugin } from "../CjsPlugin";

type NotificationType = "success" | "error" | "info" | "warning";

interface NotificationKeyframeConfig {
    name: string;
    duration: number;
    showHideOffset: number;
    yDiff: number;
}

export class CjsNotificationPlugin extends CjsPlugin {

    private readonly containerId = "cjs-notification-plugin-container";

    private readonly keyframe: NotificationKeyframeConfig = {
    ...
    private addStyles(): void {
        const theme: keyof typeof this.themes = "dark";
        const oppositeTheme: keyof typeof this.themes =
    ...
    private createContainer(): HTMLElement {
        const element = _DOMElementsUtil.HTMLToElement(`
    ...
    private createNotification(
        ...
        const svgIcons: Record<NotificationType, string> = {
        ...
        CjsTimings.sleep(this.keyframe.duration)
            .then(() => element.remove());
    }

    public info(text: string): void {
        this.createNotification(text, "info");
    }

    public error(text: string): void {
        this.createNotification(text, "error");
    }

    public warning(text: string): void {
        this.createNotification(text, "warning");
    }

    public success(text: string): void {
        this.createNotification(text, "success");
    }
